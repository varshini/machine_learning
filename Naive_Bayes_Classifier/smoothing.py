...
def calcpwv(dictionary, probdict, q) :
  global allwords
  totalwords = float(len(allwords)) # |Vocabulary|
  denominator = (sum(dictionary.values())) + q * totalwords # n + |Vocabulary|
  for word in dictionary : 
    probdict[word] = (dictionary[word] + q) / denominator
...
def classify(filelist) :
  global pwvlib, pwvcon
  error = 0
  for blog in filelist : 
    label, conscore, libscore = "", 0, 0
    blogwords = readFile(blog).splitlines()
    for word in blogwords :
      word = word.lower()
      if word in pwvcon and word in pwvlib : 
        if pwvcon[word] != 0 : conscore += math.log(pwvcon[word])
        if pwvlib[word] != 0 : libscore += math.log(pwvlib[word])
...
plib = libdocs / len(train_files)
pcon = condocs / len(train_files)

# calculate P(w|v)
qval = float(sys.argv[3])
calcpwv(liberals, pwvlib, qval)
calcpwv(conservatives, pwvcon, qval)
