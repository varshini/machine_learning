...
def curate(dictionary, cutoff) :
  sortdict = sorted(dictionary, key=dictionary.get, reverse=True)
  for i in range(int(cutoff)) :
    targetword = sortdict[i]
    del dictionary[targetword]
...
def classify(filelist) :
  ...
  for blog in filelist : 
    ...
    for word in blogwords :
      ...
      if word in liberals and word in conservatives :
...
# post-process data
curate(liberals, sys.argv[3])
curate(conservatives, sys.argv[3])
