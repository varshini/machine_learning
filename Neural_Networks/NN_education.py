...
hiddenNodes = 4 # number of nodes to use in hidden layer
iterationCounter = 0 # how many times entire sample has been iterated
outputs = [] # keep track of output values
scale = 100 # for input normalization
eta = 0.3 # step size / learning-rate
iterationNum = 2000 # how many times to iterate through training sample
    ...
    for line in range(len(readDoc)) :
      ...
      if line > 0 : # cohort members (x) 
        ...
        for value in readDoc[line].split(",") :
          member.append(float(value)/scale)
...
def bp() :
  ...
  while test_in_progress :
    ...
    if k == sampleSize :
      ...
      k = 0
      outputs = []
      # if iterationCounter % (iterationNum/10) == 0 : eta = eta * 0.85
...
def testHypothesis(testData) :
  ...
  for tester in range(len(testData)) :
    ...
    sys.stdout.write("%f\n" % round(100 * calculatedVal))
  # calcKeys = numpy.array(calcKeys)
  # testError = sum((calcKeys/scale - answerkeys/scale) ** 2) / len(testData)
